...
const server = createServer(async (req, res) => {
	...
	try {
		...
		res.statusCode = 500
		res.setHeader('Content-Type', 'application/json')
		res.end(JSON.stringify({ loc: 'createTableIfNotExists', error: error.message }));
		return;
	}
	try {
		let tempData;
		...
		if (!data.length) {
			do {
				...
				tempData = await fetch('https://rickandmortyapi.com/api/character?page=' + (++page)).then((res) => res.json());
	...
	try {
		await data.map((item) => item.map((el) => {
